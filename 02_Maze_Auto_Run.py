...
class MazeGame:
    def __init__(self, root, maps):
        ...
        self.draw_maze()
        self.root.after(500, self.auto_solve)  # เริ่มต้นให้ AI หาทางเอง
    ...
    def move(self, dy, dx):
        ...
        if 0 <= next_y < len(self.maze) and 0 <= next_x < len(self.maze[0]):
            ...
            if next_cell in (' ', 'E'):
                ...
                self.draw_maze()
                if next_cell == 'E':
                    self.root.after(500, self.next_level)  # ข้ามไปด่านต่อไป
    ...
    def next_level(self):
        ...
        if self.current_level < len(self.maps):
            self.load_level()
            self.draw_maze()
            self.root.after(500, self.auto_solve)  # เริ่มให้ AI เล่นอัตโนมัติในด่านถัดไป
        else:
            self.canvas.delete("all")
    ...
    def auto_solve(self):
        """ ใช้ BFS เพื่อหาทางออกที่สั้นที่สุด แล้วเดินตามทางนั้น """
    ...
    def follow_path(self, path):
        ...
        for dy, dx in path:
            self.root.after(200, self.move, dy, dx)  # ใช้ after เพื่อให้เดินแบบเรียลไทม์
